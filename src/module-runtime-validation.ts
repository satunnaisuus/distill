...
import { exportedBindingBrand, moduleDefinitionBrand, moduleImportWireBrand } from "./brands";
import type { AnyModuleDefinition, AnyModuleImportWire, ExportedBinding, ModuleBindingInput } from "./module-types";
...
const isExportedBindingRuntime = (value: unknown): value is ExportedBinding => {
    return typeof value === "object" && value !== null && Object.hasOwn(value, exportedBindingBrand);
};

const unwrapModuleBindingRuntime = (binding: ModuleBindingInput): AnyBinding => {
    return isExportedBindingRuntime(binding) ? binding.binding : binding;
};
...
export const assertNoImportedLocalSingleBindings = (
    ...
    for (const binding of bindings) {
        const unwrappedBinding = unwrapModuleBindingRuntime(binding);
...
const collectExportedEntries = (modules: readonly AnyModuleDefinition[]): readonly RuntimeExportedEntry[] => {
    ...
    for (const currentModule of modules) {
        for (const moduleBinding of currentModule.bindings) {
            if (isExportedBindingRuntime(moduleBinding)) {
