...
test("rejects dependency tokens with any or too-wide branded parts", () => {
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ port: anyValuePortToken }, () => ({
                port: 3000,
            })),
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ port: anyKeyPortToken }, () => ({
                port: 3000,
            })),
            bind(tokens.port).factory(() => 3000),
        ).create();
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ port: wideAnyValueToken }, () => ({
                port: 3000,
            })),
            bind(tokens.port).factory(() => 3000),
        ).create();
...
test("rejects ref dependency tokens with any or too-wide branded parts", () => {
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ port: ref(anyValuePortToken) }, () => ({
                port: 3000,
            })),
            bind(tokens.port).factory(() => 3000),
        ).create();
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ port: ref(anyKeyPortToken) }, () => ({
                port: 3000,
            })),
            bind(tokens.port).factory(() => 3000),
        ).create();
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ port: ref(wideAnyValueToken) }, () => ({
                port: 3000,
            })),
            bind(tokens.port).factory(() => 3000),
        ).create();
...
test("rejects binding tokens with any or too-wide branded parts", () => {
    expect(() => {
        defineContainer(
            tokenList,
            bind(anyValuePortToken).factory(() => 3000),
        ).create();
    }).type.toRaiseError("__token_not_in_tokens__");
    expect(() => {
        defineContainer(
            tokenList,
            bind(anyKeyPortToken).factory(() => 3000),
        ).create();
    }).type.toRaiseError("__token_not_in_tokens__");
    expect(() => {
        defineContainer(
            tokenList,
            bind(wideAnyValueToken).factory(() => 3000),
        ).create();
    }).type.toRaiseError("__token_not_in_tokens__");
});
...
test("allows legitimate any service tokens from token", () => {
    ...
    const serviceBinding = bind(anyTokens.service).factory(() => ({ port: 3000 }));
    const consumerBinding = bind(anyTokens.consumer).factory(
...
test("rejects raw any tokens used as bindings, dependencies, and refs", () => {
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(anyToken).factory(() => 3000),
        ).create();
    }).type.toRaiseError("__token_not_in_tokens__");
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ port: anyToken }, () => ({
                port: 3000,
            })),
            bind(tokens.port).factory(() => 3000),
        ).create();
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ port: ref(anyToken) }, () => ({
                port: 3000,
            })),
            bind(tokens.port).factory(() => 3000),
        ).create();
...
test("defineContainer rejects same-key tokens with incompatible value types", () => {
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(stringPortToken).factory(() => "3000"),
        ).create();
    }).type.toRaiseError("__token_not_in_tokens__");
});
...
test("defineContainer rejects same-key tokens with narrower value types", () => {
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(narrowPortToken).factory(() => 3000 as const),
        ).create();
    }).type.toRaiseError("__token_not_in_tokens__");
});
...
test("defineContainer rejects binding tokens with widened keys", () => {
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(widenedPortToken).factory(() => 3000),
        ).create();
    }).type.toRaiseError("__token_not_in_tokens__");
});
...
test("defineContainer rejects same-key dependency tokens with incompatible value types", () => {
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ port: stringPortToken }, ({ port }) => ({
                port: port.length,
            })),
            bind(tokens.port).factory(() => 3000),
        ).create();
...
test("defineContainer rejects same-key dependency tokens with narrower value types", () => {
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ port: narrowPortToken }, ({ port }) => ({
                port,
            })),
            bind(tokens.port).factory(() => 3000),
        ).create();
...
test("defineContainer rejects dependency tokens with widened keys", () => {
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ config: widenedConfigToken }, ({ config }) => ({
                port: config.port,
            })),
...
test("defineContainer rejects ref dependency tokens with widened keys", () => {
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ logger: ref(widenedLoggerToken) }, () => ({
                port: 3000,
            })),
...
test("defineContainer rejects lazy ref dependency tokens with any-typed tokens", () => {
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ logger: ref(() => anyTypedToken) }, () => ({
                port: 3000,
            })),
            bind(tokens.logger).factory(() => ({
                log() {},
...
test("defineContainer rejects lazy ref dependency tokens with same keys and incompatible value types", () => {
    ...
    expect(() => {
        defineContainer(
            tokenList,
            bind(tokens.server).factory({ logger: ref(() => sameKeyWrongValueToken) }, () => ({
                port: 3000,
            })),
            bind(tokens.logger).factory(() => ({
                log() {},
...
test("resolve uses bound value types for same-key tokens with narrower aliases", () => {
    ...
    const container = defineContainer(
        tokenList,
        bind(tokens.port).factory(() => Math.random()),
    ).create();
