...
test("bind and resolve preserve unknown service types", () => {
    const binding = bind(tokens.unknown).factory(() => ({ port: 3000 }));
...
test("bind rejects dependency factories returning values outside the token type", () => {
    expect(() => {
        bind(tokens.port).factory({ config: tokens.config }, () => "3000");
    }).type.toRaiseError();
});
...
test("bind rejects incorrectly annotated eager dependency parameters", () => {
    expect(() => {
        bind(tokens.server).factory({ config: tokens.config }, ({ config }: { readonly config: string }) => ({
            port: config.length,
        }));
    }).type.toRaiseError();
});
...
test("bind rejects incorrectly annotated ref dependency parameters", () => {
    expect(() => {
        bind(tokens.server).factory({ logger: ref(tokens.logger) }, ({ logger }: { readonly logger: Logger }) => {
            logger.log("ready");
...
test("ref preserves union token value types", () => {
    ...
    const binding = bind(tokens.server).factory({ dependency }, () => ({
        port: 3000,
    }));
