...
test("symbol token keys preserve symbol identity and value types", () => {
    ...
    expect<TokenValue<typeof Config>>().type.toBe<{ readonly port: number }>();
    expect(ref(Config)).type.toBe<RefToken<typeof Config>>();
    expect(container.resolve(Config)).type.toBe<{ readonly port: number }>();
});
...
test("symbol and class multibind tokens preserve key identity", () => {
    ...
    const SymbolHooks = multiToken(hookKey).of<{ readonly name: string }>();
    const ClassHooks = multiToken(Hook).of();

    expect(SymbolHooks).type.toBe<MultiToken<typeof hookKey, { readonly name: string }>>();
...
test("duplicate validation uses symbol identity", () => {
    ...
    const SymbolService = token(symbolKey).of<{ readonly id: "symbol" }>();
    const DuplicateSymbolService = token(symbolKey).of<{ readonly id: "duplicate-symbol" }>();
