...
describe("resolveOptional", () => {
    ...
    it("does not hide errors thrown by a registered binding factory", () => {
        const Broken = token("Broken").of<{ readonly ok: boolean }>();
    ...
    it("throws for tokens outside the token list", () => {
        const Config = token("Config").of<{ readonly port: number }>();
        const External = token("External").of<{ readonly port: number }>();
    ...
    it("respects module public visibility", () => {
        ...
        const AppModule = defineModule({
            bindings: [
                ...
                exported(bind(Public).factory({ secret: Secret }, ({ secret }) => ({ value: secret.value }))),
            ],
        });
