const tokenBrand: unique symbol = Symbol("tokenBrand");
const qualifierBrand: unique symbol = Symbol("qualifierBrand");
const qualifiedTokenBrand: unique symbol = Symbol("qualifiedTokenBrand");

type IsAny<TValue> = 0 extends 1 & TValue ? true : false;
type IfNever<TValue, TWhenNever, TWhenPresent> = [TValue] extends [never] ? TWhenNever : TWhenPresent;
...
export type TokensNotIn<TTokens extends AnyToken, TCandidates extends AnyToken> = TTokens extends AnyToken
    ? HasTrue<TCandidates extends AnyToken ? IsExactToken<TTokens, TCandidates> : false> extends true
        ? never
        : TTokens
    : never;
...
export type RuntimeTokenDetails = {
    readonly key: string;
    readonly keyId: string;
    readonly id: string;
...
export const getRuntimeTokenDetails = (currentToken: AnyToken): RuntimeTokenDetails => {
    return {
        key: tokenDisplayKey(currentToken),
        keyId: tokenKeyRuntimeId(currentToken),
        id: tokenRuntimeId(currentToken),
        isMulti: isRuntimeMultiToken(currentToken),
    };
};
...
export const assertMultiTokenKey = (currentTokenKey: string, currentToken: AnyToken): void => {
    if (!isMultiToken(currentToken)) {
        throw new Error(`Token "${currentTokenKey}" is not a multibind token`);
    }
};
